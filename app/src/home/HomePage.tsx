...
import { getStrategies, useQuery } from "wasp/client/operations";
  ...
  return (
    <ContentWrapper>
      <div className="flex justify-between">
        ...
        <button
          className="group bg-slate-100 flex border-2 border-black rounded-lg p-1 dark:bg-boxdark-2 dark:border-0"
          onClick={() => setNewProjectModalOpen(true)}
        >
          <IoMdAddCircleOutline
            size="3rem"
            className="text-slate-500 group-hover:rotate-6 group-hover:scale-110 duration-500 dark:text-white"
          />
          <span className="font-bold p-1 text-xl text-sky-700 dark:text-blue-300">
            new
          </span>
        </button>
      </div>
