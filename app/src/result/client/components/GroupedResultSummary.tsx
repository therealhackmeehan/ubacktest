...
function GroupedResultsSummary({
  resultsByStrategy,
  setResultToHighlight,
}: GroupedResultsProps) {
  const [isExpanded, setIsExpanded] = useState<boolean>(false);
  const [isDateRangeExpanded, setIsDateRangeExpanded] =
  ...
  const xToDate = (x: number) =>
    new Date((x / viewBoxWidth) * (+maxDate - +minDate) + +minDate);
  ...
  const averages = useMemo(() => {
    ...
    const result = {
      averageProfitLoss: count
        ? total(parsedResults, "pl", "profit/loss") / count
        : 0,
      averageProfitLossWCosts: count
        ? total(parsedResults, "plWCosts", "profit/loss (w/ trading costs)") /
          count
        : 0,
      averageCagr: count ? total(parsedResults, "cagr", "CAGR") / count : 0,
      averageNumTrades: count
        ? total(parsedResults, "numTrades", "number of trades") / count
        : 0,
      averageNumProfitableTrades: count
        ? total(parsedResults, "numProfTrades", "number of profitable trades") /
          count
        : 0,
      averagePercTradesProfitable: count
        ? total(parsedResults, "percTradesProf", "% trades profitable") / count
        : 0,
      averageSharpeRatio: count
        ? total(parsedResults, "sharpeRatio", "sharpe ratio") / count
        : 0,
      averageSortinoRatio: count
        ? total(parsedResults, "sortinoRatio", "sortino ratio") / count
        : 0,
      averageMaxDrawdown: count
        ? total(parsedResults, "maxDrawdown", "max drawdown") / count
        : 0,
      averageMaxGain: count
        ? total(parsedResults, "maxGain", "max gain") / count
        : 0,
      averageMeanReturn: count
        ? total(parsedResults, "meanReturn", "mean return") / count
        : 0,
      averageStdDevReturn: count
        ? total(parsedResults, "stddevReturn", "std dev return") / count
        : 0,
      averageMaxReturn: count
        ? total(parsedResults, "maxReturn", "max return") / count
        : 0,
      averageMinReturn: count
        ? total(parsedResults, "minReturn", "min return") / count
        : 0,
      averageTimepoints: count
        ? total(parsedResults, "length", "timepoints") / count
        : 0,
    };
    ...
    <>
      <div className="border-b-2 border-black/30 dark:border-white/30 flex justify-between items-center">
        ...
        <button
          ...
          className={`flex items-center px-1 space-x-1 bg-slate-50 border-2 border-slate-100 hover:scale-90 ${
            resultsByStrategy.length <= 1 ? "cursor-not-allowed opacity-50" : ""
          }`}
        >
        ...
        <div className="rounded-b-xl border-x-2 border-t-4 border-b-2 border-black/30 dark:border-white/30 bg-slate-50 mb-4 p-2 shadow-lg dark:bg-boxdark-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 p-2 md:p-4">
            <StatItem
              ...
              unit="%"
            />
            {averages.averageProfitLoss !==
              averages.averageProfitLossWCosts && (
              <StatItem
                label="Average P/L (with costs)"
                value={averages.averageProfitLossWCosts}
                unit="%"
              />
            )}
            <StatItem
              label="Average CAGR"
              value={averages.averageCagr}
              unit="%"
            />
            <StatItem
              label="Average # of Timepoints"
              value={averages.averageTimepoints}
            />
            <StatItem
              label="Average # of Trades"
              value={averages.averageNumTrades}
            />
            <StatItem
              label="Average # of Profitable Trades"
              value={averages.averageNumProfitableTrades}
            />
            <StatItem
              label="% of Trades Profitable"
              value={averages.averagePercTradesProfitable}
              unit="%"
            />
            <StatItem
              label="Average Sharpe Ratio"
              value={averages.averageSharpeRatio}
              precision={3}
            />
            <StatItem
              label="Average Sortino Ratio"
              value={averages.averageSortinoRatio}
              precision={3}
            />
            <StatItem
              label="Average Max Drawdown"
              value={averages.averageMaxDrawdown}
              unit="%"
            />
            <StatItem
              label="Average Max Gain"
              value={averages.averageMaxGain}
              unit="%"
            />
            <StatItem
              label="Average Mean Return"
              value={averages.averageMeanReturn}
              unit="%"
            />
            <StatItem
              label="Average Std Dev of Return"
              value={averages.averageStdDevReturn}
              unit="%"
            />
            <StatItem
              label="Average Max Return"
              value={averages.averageMaxReturn}
              unit="%"
            />
            <StatItem
              label="Average Min Return"
              value={averages.averageMinReturn}
              unit="%"
            />
          </div>
          ...
          <div className="hidden md:flex justify-between m-1">
            ...
            <div className="rounded-md p-1 bg-slate-200 border-2 border-slate-300">
              <div className="text-xs italic font-extralight text-slate-700 font-sans">
                <span className="font-bold">{tickerToDisplay}</span> @{" "}
                {dateToDisplay}
              </div>
            </div>
            <button
          ...
          {isDateRangeExpanded && (
            <>
              <div className="w-full h-32 overflow-y-auto border-2 dark:border-0 border-slate-200 bg-white dark:bg-boxdark mt-4">
                <svg
                  ...
                  onMouseMove={(e) => {
                    ...
                    const svgX =
                      ((e.clientX - bounds.left) / bounds.width) * viewBoxWidth;
                    setHoverX(svgX);
                ...
                >
                  {stackedLevels.map(({ range, level }, i) => {
                    const x1 =
                      ((+range.start - +minDate) / (+maxDate - +minDate)) *
                      viewBoxWidth;
                    const x2 =
                      ((+range.end - +minDate) / (+maxDate - +minDate)) *
                      viewBoxWidth;
              ...
              <div className="text-xs font-light text-end italic mt-2 dark:text-slate-100">
                Each line corresponds to a backtest result for this strategy,
                listed below. Hover over a line to display the specific strategy
                details. Green lines = profitable backtests, while red lines =
                losses.
              </div>
            </>
          )}
