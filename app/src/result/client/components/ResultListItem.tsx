...
export default function ResultListItem({
  ...
  const [deleteResultModalOpen, setDeleteResultModalOpen] =
    useState<boolean>(false);
  const [shareResultModalOpen, setShareResultModalOpen] =
    useState<boolean>(false);
  ...
  return (
    <>
      ...
      {shareResultModalOpen && (
        <ShareResultModal
          closeModal={() => setShareResultModalOpen(false)}
          id={result.id}
        />
      )}
      {renameResultModalOpen && (
        <RenameResultModal
          closeModal={() => setRenameResultModalOpen(false)}
          id={result.id}
          currResultName={result.name}
        />
      )}
      {deleteResultModalOpen && (
        <DeleteResultModal
          closeModal={() => setDeleteResultModalOpen(false)}
          id={result.id}
        />
      )}
