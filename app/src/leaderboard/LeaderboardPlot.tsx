import CandlePlot from "../editor/client/components/result/plots/candle/CandlePlot";
...
function LeaderboardPlot({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center overflow-y-auto z-50">
      <div
        className="fixed inset-0 w-full bg-black/70"
        onClick={() => setLeaderboardPlotOpen(false)}
      ></div>
        ...
        <div className="min-w-187.5">
          <div className="flex justify-between m-2 p-4 bg-gradient-to-r from-sky-700 to-white shadow-lg rounded-md">
            <div className="font-extrabold text-slate-100 text-sm md:text-xl tracking-tight flex">
              Strategy Overview{" "}
              <span className="hidden lg:flex text-xs p-2 bg-slate-700 text-white italic font-extralight mx-4 rounded-full">
            ...
            <div className="flex gap-x-4 items-end text-sm md:text-base">
              <div>
                Rank:{" "}
                <span className="bg-sky-700 text-white rounded-lg py-1 px-2 m-1">
                  {index}
                </span>
              </div>
              <div>
                P/L:{" "}
                <span className="bg-sky-700 text-white rounded-lg p-1 m-1">
                  {result.pl?.toFixed(2)}%
                </span>
              </div>
              <div>
              ...
              <button
                className="text-red-500 hover:-rotate-90 duration-700"
                onClick={() => setLeaderboardPlotOpen(false)}
              >
