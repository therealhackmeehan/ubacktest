...
class ResultValidator {
  public static validatePortfolio(strategyResult: StrategyResult): void {
    // for each of the crucial columns, check they all match in length
    const requiredKeys: (keyof StrategyResult)[] = [
      "portfolio",
      "portfolioWithCosts",
      "signal",
      "returns",
    ...
    const portfoliosNonNegative =
      strategyResult.portfolio.every((value) => value >= 0) &&
