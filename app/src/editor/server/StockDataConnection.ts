...
class StockDataConnection {
  ...
  private readonly minutesPerInterval: Record<
    (typeof intVals)[number],
    number
  > = {
    "1min": 1,
    "5min": 5,
  ...
  private processStockData(apiResult: any) {
    ...
    const { normalizedQuote, shortenedNormalizedQuote } =
      this.normalizeAndShortenData(quote);
  ...
  private validateData(quote: QuoteColumns): void {
    const missingClosePricesMsg =
      "Stock exists but no data found. Try another stock or timeframe.";
    if (this.isEOD) {
