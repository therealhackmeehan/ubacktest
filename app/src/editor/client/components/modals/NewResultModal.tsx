...
export default function NewResultModal({
  onSuccess,
  closeModal,
  symbol,
}: NewProjectModalProps) {
  const currDate = new Date();
  const currDateString: string = currDate
    .toLocaleDateString()
    .replaceAll("/", "_");
  ...
  const [newResultName, setNewResultName] =
    useState<string>(fullPlaceholderName);
    ...
    <ModalLayout closeModal={closeModal}>
      <div className="flex justify-between">
        <h2 className="text-base text-slate-500 font-semibold dark:text-white">
          Save Result{" "}
          <span className="text-slate-800 dark:text-blue-300">
            To My Results
          </span>
        </h2>
