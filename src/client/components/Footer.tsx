...
export default function Footer() {
  return (
    ...
    >
      ...
      <div className='p-4 w-full text-center md:text-start place-self-end text-slate-500 dark:text-white/50 pb-6 text-xs md:pb-0 md:text-base'>© 2025 uBacktest. All rights reserved.</div>
        ...
        {Object.entries(footerLinks).map(([section, links]) => (
          <div key={section}>
            ...
            <ul className='mt-6 space-y-2'>
              {links.map(({ name, href }) => (
                <li key={name}>
                  <a href={href} className='text-sm text-gray-600 hover:text-slate-700 dark:text-white text-nowrap'>
                    {name}
                  </a>
                </li>
              ))}
