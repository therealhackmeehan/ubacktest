...
function LeaderboardItem({ result, index }: LeaderboardItemProps) {
  ...
  return (
    <>
      <div className="border-2 md:border-0 md:border-l-8 border-sky-700/50 dark:border-blue-300 rounded-md my-6 md:my-2 bg-white p-2 justify-between md:flex duration-700 dark:bg-black">
        <div className="px-3 mt-6 md:mt-0 flex items-center gap-x-3">
          <div className="text-title-lg font-bold dark:text-white">
            {index + 1}
          </div>
          <div
            className={`py-2 px-4 rounded-xl ${randomColor()} text-white font-extrabold text-xl dark:text-black`}
          >
        ...
        <div className="font-bold text-xs text-end tracking-tight md:flex gap-x-3 justify-end space-y-2 md:space-y-0">
          <div className="p-2 m-1 text-sky-700 dark:text-blue-300 text-xl md:text-xs">
            @{result.email}
          </div>
          <div className="flex gap-x-4 justify-between py-1 px-2 rounded-md bg-slate-100 dark:bg-boxdark dark:text-white">
            CAGR{" "}
            <span className="text-lg text-sky-700 font-mono dark:text-blue-300">
          ...
          <button
            className="px-3 py-1 w-full md:w-auto flex rounded-md bg-slate-300 hover:shadow-lg items-center gap-x-2 dark:bg-white text-lg md:text-xs"
