    ...
    <ModalLayout closeModal={closeModal}>
      <div className="flex justify-between">
        <h2 className="text-base text-slate-500 font-semibold dark:text-white">
          Share your result with{" "}
          <span className="dark:text-blue-300">another user</span>!
        </h2>
        ...
        <div className="m-2 p-2 dark:text-white text-sm">
          ...
          <ul className="p-2">
            {sharedWith.map((sharedWithEntry, index) => (
              <li
                className="text-center text-sky-700 dark:text-blue-300"
                key={index}
              >
                {(sharedWithEntry as GetSharedWithWithReceiver).receiver
                  .email ?? "unknown"}
              </li>
            ))}
          </ul>
      ...
      {currentlyInTimeout && (
        <div className="mt-4">
          <div className="h-2 bg-gray-300 rounded-full">
            <div
              className="h-full bg-sky-700/50 rounded-full"
