                ...
                <>
                    <div className="text-end">
                        <label className="inline-flex items-center cursor-pointer">
                            <input data-testid="group-by-parent" type="checkbox" value="" className="sr-only peer" checked={groupByStrategy} onChange={() => setGroupByStrategy(!groupByStrategy)}></input>
                            <div className="relative w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-sky-700 dark:peer-checked:bg-blue-300"></div>
                            <span className="ms-3 text-sm font-medium text-gray-900 dark:text-white">Group By Parent Strategy</span>
                        </label>
                    </div>
                    {groupByStrategy ? (
                        Object.entries(groupResultsByStrategy(results)).map(([strategyId, resultsByStrategy]) => (
                            <StrategySpecificResultList strategyId={strategyId} resultsByStrategy={resultsByStrategy} resultToHighlight={resultToHighlight} setResultToHighlight={setResultToHighlight} />
                        ))
                    ) : (
