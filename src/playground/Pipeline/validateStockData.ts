interface ValidateStockDataProps {
    returnedStockData: any;
  }
  
  function validateStockData({ returnedStockData }: ValidateStockDataProps) {
    if (!returnedStockData) {
      throw new Error("Stock Data has returned as undefined. Please try again.");
    }
  
    // Check for error message in response
    const errMsg = returnedStockData.chart?.error;
    if (errMsg) {
      throw new Error("Stock Data retrieval failed: " + errMsg);
    }
  
    // Check that 'chart.result' is an array with exactly one item
    if (!Array.isArray(returnedStockData.chart?.result) || returnedStockData.chart.result.length !== 1) {
      throw new Error("Unexpected number of results returned for that symbol. Please try again.");
    }
  
    const result = returnedStockData.chart.result[0];
